module.exports = function (redis) {
  ...
  redis.defineCommand('autojoinEvent', {
    ...
    lua: `
      ...
      local userCount = tonumber(ARGV[3])
      local options = ARGV[4]
      ...
      for i, eventKey in ipairs(eventKeys) do
        ...
        if eventJSON == false then
          ...
          if event.userCount == userCount and
             event.options == options then
            ...
            if not hasJoined then
              ...
              if canJoin then
                ...
                if usersJoinedCount + 1 == event.userCount then
                  redis.call('srem', pendingKey, eventKey)
